...
| Variable | Source | Retrieval Method |
|------|------|---------|
| `{disasm_for_reference}` | Legacy IDA | `idapro_disasm(addr="function name")` |
| `{procedure_for_reference}` | Legacy IDA | `idapro_decompile(addr="function name")` |
| `{disasm_code}` | New IDA | `idapro_disasm(addr="corresponding address")` |
| `{procedure}` | New IDA | `idapro_decompile(addr="corresponding address")` |
| `{symbol_name_list}` | Legacy Extraction | Extract all symbol names that are not `sub_` or `loc_` from the reference code |

...
- Ghidra 脚本（Java/Python Jython/PyGhidra）自动化；Jython 已过时，新脚本只用 PyGhidra
...
| 工具 | 用途 | 自举 |
|------|------|------|
| Ghidra 12.1.3（现行 PUBLIC；需 64 位 JDK 21） | 反编译主工具 | 手动 release / 包管理器 |
| PyGhidra（随 Ghidra 附带） | Python 3 自动化（support/pyghidraRun） | 装 pypkg/dist 下的 wheel，绝不从 PyPI |

...
**Core rule: Always use the `file` command or magic bytes to identify the file type. Do not trust the file extension.**

Common disguise methods:

| Disguised extension | Actual type | Purpose |
|---------|---------|------|
| `.sh` | ELF binary | Make people think it is a script and lower their guard |
| `.txt` | PE/ELF | Bypass simple file type filters |
| `.jpg`/`.png` | Executable file or archive | Hide it in an image |
| `.dll` | Actually a .NET assembly | Confuse the analysis direction |
| `.so` | Actually an encrypted payload | Decrypt it first |
...
# Use xxd to view the magic bytes
...
**"DriverLoader" does not always load a driver. "Updater" does not always update.**
...
**During analysis:**
- Ignore the name implication. Judge the file by its actual code behavior.
- Focus on system calls such as `mmap`, `ptrace`, and `/proc/self/mem`.
- If you see "load driver" but no `insmod`/`init_module` call, the name is misleading.

### Dynamic Analysis When Static Analysis Is Not Enough

Static analysis alone can see only the code structure. Use dynamic analysis in the following cases:

| Scenario | Recommended Dynamic Method |
|------|-------------|
| The code has decryption or decompression logic | Set a breakpoint after decryption and dump the plaintext |
| Many indirect calls, such as a function pointer table | Use strace/ltrace to trace actual calls |
| Suspected anti-debugging | First use strace to check ptrace calls |
| Embedded shellcode or payload | Use QEMU user-mode emulation to execute it |
| Unknown network communication protocol | Use tcpdump/Wireshark for packet capture |

```bash
# Use strace to trace system calls (focus on relevant calls)
...
# Check for anti-debugging: see whether it self-traces with ptrace
strace ./binary 2>&1 | grep ptrace
# If you see ptrace(PTRACE_TRACEME, ...) it indicates anti-debugging
```

### Common Patterns in Process Injection and Packer Protection Samples

These samples, such as `LinYuDriverLoader`, usually:

1. **Do not actually load a kernel driver** (root privileges are required, but most scenarios do not have them)
2. **Actually perform process injection**:
   - `ptrace` attach to the target process
   - Read and write target memory through `/proc/<pid>/mem`
   - Map shellcode into the target process space with `mmap`
3. **Contain an encrypted payload**:
   - Decrypt a shellcode block at run time
   - The decrypted payload is the actual hook code
4. **Use anti-debugging protection**:
   - `ptrace(PTRACE_TRACEME)` self-tracing
   - Time check (compare values before and after `clock_gettime`)
   - Check TracerPid in `/proc/self/status`

**Analysis strategy**:
```text
1. Use the file command to confirm the actual type
2. Use strings to check for obvious paths, library names, or error messages
3. Use rabin2 -I to check the architecture, compiler, and protections
4. Statically find mmap, ptrace, and open calls

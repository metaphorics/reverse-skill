...
.NET managed programs (C# compiled exe/dll) have a complete independent workflow. See [`../dotnet-reverse/SKILL.md`](../dotnet-reverse/SKILL.md) for details.

**Quick reference:**
- Identify: PE header CLR runtime header is non-zero, `mscoree.dll` imports, `mscorlib`/`System.Private.CoreLib` strings
- Tools: **dnSpyEx** (decompilation + debugging + IL editing, Windows GUI) / **ILSpy** (CLI) / **de4dot** (deobfuscation) / **dnlib** (scripting)
- Deobfuscation: ConfuserEx / SmartAssembly / Babel / .NET Reactor → `de4dot`. See [`../dotnet-reverse/references/obfuscators.md`](../dotnet-reverse/references/obfuscators.md) for details.
- **IL first**: Use an IL editor, not a C# decompiler, for key decisions and patches. This avoids distortion from recompiling compiler-generated code.
- Analyze red-team Sharp* tools (Rubeus/SharpHound) → [`../dotnet-reverse/references/sharp-tools.md`](../dotnet-reverse/references/sharp-tools.md)

**Boundary:** IL2CPP / NativeAOT compiled outputs are native (no CLR). Do not use dnSpy. Use `ida-reverse/` / `radare2/` instead (see [seed-014](../field-journal/seed-014_unity-il2cpp-reverse.md)). The Codegate 2013 two-stage XOR+AES-CBC pattern is in the `.NET Analysis` section of [tools.md](tools.md#net-analysis).

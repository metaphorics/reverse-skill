...
# reverse-skill routing regression tester: runs all routing-benchmark.json cases and checks the PRIMARY expectation.
# Usage:
...
# Exit codes: 0 all cases pass, 1 at least one case failed (failure details are written to LogDir)
param(
    [string] $Benchmark = '',
    [switch] $Quick,
    [string] $LogDir = '',
...
# $env:TEMP can be unset on Linux/macOS runners, so always fall back to GetTempPath

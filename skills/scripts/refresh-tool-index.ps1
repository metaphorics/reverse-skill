...
$markdownLines = @(
    ...
    '- Routing entry: `SKILL.md` → `routing.md` → matching sub-skill',
    '- Note: `skills/scripts/refresh-tool-index.ps1` generates this table automatically. Agent clients use it to confirm routes and tool paths.',
    '- Note: Calculate tool availability and runtime separately for MCP-only capabilities. `npx` only indicates the npm MCP runtime condition. It does not make jshookmcp or reqable-mcp available or Ready by itself.',
    '',
...
$markdownCapLines = @(
    ...
    '---',
    '',
    '## Capability status view (Capability Status)',
    '',
    '| Capability | Tool available | Ready | MCP registered | Service online | MCP HTTP | Auto-installable | Install method |',
    '|------|---------|-------|-----------|---------|-----------|---------|'
)

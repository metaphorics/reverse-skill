...
# --- New source and artifact checks (routing.json / benchmark / INDEX) ---
...
if (Test-Path -LiteralPath $benchJson) {
    ...
    # Benchmark expect IDs must exist in routing.json (prevent references to deleted routes).
    if (Test-Path -LiteralPath $routingJson) {
...
# master-route.ps1 must not fall back to a hardcoded routing table (prevent bypassing routing.json).
...
foreach ($rp in @($rulesEn)) {
    ...
    if ($rt -match 'auth\.status\s*=\s*granted|auth.status=granted|MUST NOT ACT against targets') {
...
Assert-Fields (Join-Path $skillsRoot 'reverse-engineering/references/re-agent-workflow.md') @('Triage', 'Static', 'Dynamic', 'Synthesis', 'E-iat-repair-fail', 'E-exports', 'dnSpy', 'E-self-check-crash', 'ExitProcess', 'E-api-hash', 'E-anti-debug-peb', 'E-wide-strings', 'A–T', 'U–AV', 'nonpe-format-cookbook')
...
Assert-Fields (Join-Path $skillsRoot 'docs-generator/SKILL.md') @('vendor-report-rules.md', 'flavor = null')
Assert-Fields $vendorRulesPath @('flavor = null', 'explicit_malware')
Assert-Fields $vendorRulesPath @('skills/ops/evidence-finding-path.md', 'securelist.com/updated-mata', 'www.huorong.cn', 'thin overlay', 'vuln')
Assert-Fields (Join-Path $skillsRoot 'malware-analysis/SKILL.md') @('E-iat-repair-fail', 'E-exports', 'E-self-check-crash', 'ExitProcess', 'E-api-hash', 'E-sig-forge', 'A–T', 'U–AV', 'E-batch-deobf', 'E-vba-pcode')
Assert-Fields (Join-Path $skillsRoot 'reverse-engineering/anti-analysis.md') @('E-anti-debug-cpuid', 'E-api-hash', 'SigCheck', 'ollvm-deobfuscation')
...
Assert-Fields (Join-Path $skillsRoot 'docs-generator/references/security-report-templates.md') @('thin `vuln`')
...
# Use one of pinnedVersion, pinnedCommit, or pinPolicy.
# github-release-* also accepts assetSha256 or preferApiDigest (GitHub release asset hashes).
# local-http-mcp can omit a pin only when it does not fetch external source.
...
foreach ($mf in @($skillsManifest, $kaliManifest)) {
    ...
    foreach ($cap in $mc.capabilities) {
        ...
        switch ($capMap['bootstrapKind']) {
            ...
            'winget-package' { $hasPin = $hasPin } # winget-latest uses pinPolicy
            'apt-package' { $hasPin = $true }      # Distribution repository provides the pin (Kali side).
...
if ($id -match 'NOT|not a Z3r0|FastAPI|React') { Ok 'identity distinguishes platform' } else { Bad 'identity weak' }
if ($id -match 'tool-index|bootstrap|field-journal') { Ok 'identity keeps reverse-skill DNA' } else { Bad 'identity missing DNA' }
...
if ($contribText -like "*genuine decision boundary*") { Ok "new-skill contract uses genuine decision boundaries" } else { Bad "new-skill contract missing genuine decision boundaries" }
...
if ($reWorkflowText -like "*decision_delta*" -and $reWorkflowText -like "*carry_forward_refs*") { Ok "representative RE workflow consumes delta by reference" } else { Bad "representative RE workflow missing delta consumer contract" }
